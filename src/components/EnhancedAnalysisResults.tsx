...
import { ArrowLeft, Download, Copy, Target, TrendingUp, Award, Star } from 'lucide-react';
...
const EnhancedAnalysisResults: React.FC<EnhancedAnalysisResultsProps> = ({ analysis, onReset }) => {
  const selectedProfile = jobProfiles.find(p => p.id === analysis.selected_profile);
  ...
  const handleDownloadReport = () => {
    ...
    a.href = url;
    a.download = `resume-analysis-${analysis.selected_profile}-report.json`;
...
Target Profile: ${selectedProfile?.title}

Overall Score: ${analysis.resume_score}/100
Profile Match: ${analysis.profile_match_score}/100
        ...
        <div>
          ...
          {selectedProfile && (
            <div className="flex items-center space-x-3 mb-2">
              <div className={`w-8 h-8 rounded-full bg-gradient-to-r ${selectedProfile.color} flex items-center justify-center text-sm`}>
                {selectedProfile.icon}
              </div>
              <span className="text-blue-200">Analyzed for: <span className="text-white font-medium">{selectedProfile.title}</span></span>
            </div>
          )}
          <p className="text-blue-200">AI-powered career assessment with profile-specific insights</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        >
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              ...
              <p className="text-blue-200 text-sm">Overall quality</p>
            </div>
          </div>
        ...
        >
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              <h3 className="text-lg font-semibold text-white">Profile Match</h3>
              <p className="text-blue-200 text-sm">Role alignment</p>
            </div>
          </div>
          <div className="text-3xl font-bold text-white">{analysis.profile_match_score}/100</div>
        </motion.div>
        ...
        >
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              ...
              <p className="text-blue-200 text-sm">Technical fit</p>
            </div>
          </div>
          <div className="text-3xl font-bold text-white">{analysis.skills.length}/{selectedProfile?.skills.length || 10}</div>
        </motion.div>
      </div>
